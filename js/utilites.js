...
var frmRegistrationURL = "/apex/websiteUtilities#!/exams/frmMemberRegister";
var erpRegistrationURL = "/apex/websiteUtilities#!/exams/erpMemberRegister";
var frmSwitchRegistrationURL = "/apex/websiteUtilities#!/exams/frmMemberSwitchRegister";
var erpSwitchRegistrationURL = "/apex/websiteUtilities#!/exams/erpMemberSwitchRegister";
var membershipRegistrationURL = "/apex/websiteUtilities#!/";

var communityDom = window.location.hostname;
var dashboardURL = communityDom + "/apex/sfdcApp#!/";

if(communityDom.indexOf("build") > -1) {
  var membershipURL = "http://build-altest.cs16.force.com";
  var contactsURL = "https://build-garpbuild.cs16.force.com";
} else if(communityDom.indexOf("uat") > -1) {
  var membershipURL = "http://uat-altest.cs15.force.com";
  var contactsURL = "https://uat-garp.cs15.force.com";
}

var digitalBadgeFrmURL = "/test/DigitalBadgeFRM";
var digitalBadgeErpURL = "/test/DigitalBadgeERP";
var publicProfile = "/test#!/frm/contacts/contacts/15773";
var bookStoreURL = "/apex/websiteUtilities#!/store/list";
var directoryOptOptURL = "/test/DirectoryOptOut";
var contentFeedURL = "/apex/contentChatter?contentId=";
var deepLinkURL = "/apex/sfdcApp?start=";
var uploadURL = "/apex/uploadAttachment?parentId=";
var chatterProfileURL = "/_ui/core/userprofile/UserProfilePage?u={0}&tab=sfdc.ProfilePlatformFeed";

var CERT_REPLACE_PRICE = 100;
var CANADIAIN_DUTY_PRICE = 16;
var ADDRESS_LINES_ALLOWED = 3;
var SIZE_ADDRESS_LINE_MAX = 32;
var TAX_RATE = .07;

var FRM1_PROD_KEY = "FRM1H";
var FRM2_PROD_KEY = "FRM2H";
var ERP_PROD_KEY = "ENC";
var MISC_PROD_CODE = 'MISC';
var DEFERAL_FRM_PROD_CODE = 'FRM1';
var DEFERAL_ERP_PROD_CODE = 'ENC';
var FBR_MEM_PROD_CODE = 'FCBR';
var FBR_NONMEM_PROD_CODE = 'FBRNM';
var FBR_BOOK_PROD_CODE = 'FBRHB';
var ICBRR_NONMEM_PROD_CODE = 'CBRNM';
var ICBRR_MEM_PROD_CODE = 'CBR';
var ICBRR_RETAKE_PROD_CODE = 'CBR2A';
var ICBRR_BOOK_PROD_CODE = 'CBRHB';

// Make same as ICBRR book above
var CERT_PROD_CODE = 'CBRHB';
var MEMBER_PROD_CODE = 'MEMI';
var MEMBER_STUDENT_PROD_CODE = 'MEMS';
var SHIP_PROD_CODE = 'SHIP';
var TAX_PROD_CODE = 'SLSTX';
var PAY_DEFERRED_PROD_CODE = "PRFEE";
var JWILEY_PROD_CODE = 'MEMW';

var MAY_EXAM_REG_GL = "4001";
var NOV_EXAM_REG_GL = "4002";
var FBR_GL = "4000";
...
var ICBRR_GL = "4000";
var JWILEY_GL = '2007';
var DEFERRED_GL = "4020";
var PAY_DEFERRED_GL = "6110";
var SHIP_GL = "4999";
var TAX_GL = "2819";

var loc = communityDom.indexOf('.');
if(loc > -1) {

  var env = communityDom.substring(0,loc);
  var domain = communityDom.substring(loc+1);

  var locEnv = env.indexOf('-');
  if(locEnv > -1) {  
    env = env.substring(0,locEnv);
    var registrationDom = env + '-altest.' + domain + '/test';
    var paymentDom = env + '-altest.' + domain + '/pmtx';
  } else {
    var registrationDom = 'altest.' + domain + '/test';
    var paymentDom = 'altest.' + domain + '/pmtx';    
  }

}

function navigate(route, params, $location, $state) {
  $('.row').hide();
  var selector = '#spin';
  var obj = $(selector)
...
function disableToggleForm(selectorClass, disabled) {
...
function startSpinner(selector, color) {
  var obj = $(selector);
  var spinOptions = jQuery.extend(true, {}, opts);       
  ...
  if(defined(obj,"length") && obj.length > 0) {
    ...
    spinOptions.top = 10;
    var spinner = new Spinner(spinOptions).spin(obj[0]);
    return spinner;
  } else {
    return null;
  }
...
function declineOrderByCountry(userData) {

  var country = '';
  for(var i=0; i<2; i++) {
    switch(i) {    
      case 0: country = userData.contactData.MailingCountry;
      break;
      case 1: country = userData.accountData.BillingCountry;
      break;
    }
...
function convertCPECreditFieldLable(remoteDataService, panelValues, panelRecord, item, labelText) {

  var pd = remoteDataService.userData.providerData;
  var val = panelValues['CPE_Activity_Type__c'];
  var match = _.findWhere(panelRecord.fieldRecords, {name: item.name});
  var matchId = _.findWhere(remoteDataService.userData.activityTypeData, {Name: val});
  if(defined(matchId)) {
    if(defined(matchId,labelText)) {
      var match = _.findWhere(panelRecord.fieldRecords, {name: item.name});
...
function isOppPaid(oppStageName) {
  return oppStageName.indexOf("Closed")+1;
}

function isPaid(contract) {
  if(defined(contract,"Opportunity__r.StageName")) {
...
function payOpportunity(contractOpp, remoteDataService, $window) {
  if(defined(contractOpp,"Opportunity__c")) {
    var payId = findOpportunityPaymentId(contractOpp, remoteDataService);    
  } else if(defined(contractOpp,"pymt__Payments__r.length") && contractOpp.pymt__Payments__r.length > 0) {
  ...
  $window.location.href = 'http://' + paymentDom  + '?pid=' + payId + '&context=website&finish_url=https%3A%2F%2F' + dashboardURL;
}

function findOpportunityPaymentId(contract, remoteDataService) {

  if(defined(contract,"Opportunity__c")) {
    for(var i=0; i<remoteDataService.userData.opportunityData.length; i++) {
      ...
      if(opp.Id == contract.Opportunity__c) {
        if(defined(opp,"pymt__Payments__r.length") && opp.pymt__Payments__r.length > 0) {
...
function findDraftContract(recordTypeName, remoteDataService) {
...
function findActiveContract(recordTypeName, remoteDataService) {
...
function findCurrentContract(recordTypeName, remoteDataService) {
  ...
  if(defined(recordTypeId)) {
    ...
    if(defined(conts)) {
      for(var i=0; i<conts.length; i++) {
        var cont = conts[i];
        if(cont.Status.indexOf('Activated') > -1 || cont.Status.indexOf('Draft') > -1) {
          return cont;
        }
      }
    }
...
function getUPSPrice(contactData, prods, countryCode, remoteDataService, callback) {
  var shippingOptions = [];
  remoteDataService.getUPSPrice(contactData.MailingStreet,'',contactData.MailingCity,countryCode,contactData.MailingState,contactData.MailingPostalCode, prods, function(err, data) {
...
function validateAddress(recordData, streetField) {
...
function getEpochShortDateTimeText(epochDate) {
...
function getEpochDateText(epochDate) {
  if(epochDate !== null && typeof epochDate !== "undefined") {
...
function getEpochDateTimeTextTime(epochDate) {
  if(epochDate !== null && typeof epochDate !== "undefined") {
...
var caData = STATES.Countries[0].s;
var usData = STATES.Countries[1].s;

// Setup Country Select
var countriesFieldPanel = [];
for(var i=0; i < countries.length; i++) {
  ...
  countriesFieldPanel.push(obj);
}

// Setup Country Select
var usStatesFieldPanel = [];
for(var i=0; i < usData.length; i++) {
  ...
  usStatesFieldPanel.push(obj);
}
// Setup Country Select
var caProvenceFieldPanel = [];
